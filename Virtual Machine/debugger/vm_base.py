...
class VirtualMachineBase:
    ...
    def remove_watchpoint(self, address):
        """
        Remove a watchpoint on a memory address.
        Args:
    ...
    def check_watchpoints(self):
        ...
        Output:
            The address of the first watchpoint that changed, or None if none changed.
        """
        for address in self.watchpoints:
            current_value = self.read_memory(address)
            if current_value is not None:
                if current_value != self.watchpoint_values.get(address):
                    self.watchpoint_values[address] = current_value
                    return address
        return None
    # [/watch]
